...
**Decision:** D11 still requires an exercise path on a user-facing stem. D30 adds the sticky row. Each operable Acceptance line has an unchecked `## Outcomes` row. Child tasks stay flat and carry `outcome: <slug>`. Completing the children does not check the outcome or that Acceptance line. Only the outcome audit may check them, and only when a Completed exercise item cites a path, a date, and an observation that the scenario held and the outcome is still open. A passing note checks the outcome even when other children remain. An outcome already checked stays checked. The check waits until after work-verifier pass when that step is in the turn, because the verifier fails a unit that checks the row. A falsely checked Acceptance line is unchecked in that same pass. A Completed break note does not spawn a second Exercise in that same pass; it spawns cited follow-ups. An empty High/Medium/Low list while an outcome is open is not stem-drained. The sync pass `optional-todo-outcomes` retrofits existing stems; it does not invent the remaining path from a code diff. Doc-roles are optional. Declining them, and a one-off change with no orchestrator, still run that audit in the parent session before a feature, stem, or outcome is called done, and when the last child or an exercise item just finished. The audit fills the next blank. It is also the only creator of a human-verify playtest, and only after that passing note.

**Do not:** Nest children under a parent checkbox. Do not let a slice check operable Acceptance. Do not treat warden `clean` plus an open outcome as feature done. Do not skip the audit because doc-roles are declined. Do not let the orchestrator or implementer write a human-verify playtest outside the audit. Do not ask the human to look at an outcome that is still open.
