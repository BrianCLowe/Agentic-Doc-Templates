...
| **Orchestrator git** | `local` · `branch-pr` *(draft mid-run → build-verify → mark ready)* · `branch-pr-squash` *(+ squash before ready for tip-only bots)* · `branch-push` · `current-push` · `none` — **always ask if unset**; after pick, probe forge CLI from remote — **ask to install** if missing, **ask to start auth** if not logged in (install ≠ PR-ready) | Step 3p / B0.6 · `orchestrator.git.mode` · [Git policy](../agent/roles/orchestrator.md) |
