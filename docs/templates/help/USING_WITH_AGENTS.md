...
| **Team roster** | Who exists + job + handoff. Create `docs/Team-Roster.md` **only** when team inbox is on. Coding agents on a handoff **read** (do not invent rows). Bots **self-ID**; report-only bots ask another agent to add them | [`Team_Roster_Template.md`](../Team_Roster_Template.md) · same §13 |
