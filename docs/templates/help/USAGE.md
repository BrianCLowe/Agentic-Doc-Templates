...
| Optional role — orchestrate | *Orchestrate — clear ready TODOs until blocked.* *(parent loop; git from `orchestrator.git.mode` — recommend branch-pr-squash / or branch-pr / …; close-out: build-verify → todo-warden → squash? → mark ready; Human-TODO verify map)* |
