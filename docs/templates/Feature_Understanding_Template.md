...
4. Set `confirmed` only after the user approves **shape**. Then graduate contract to the spec (Workflow §2). When `confirmed`, do **not** re-ask for Understanding review unless shape/scope changes. Plans/TODOs must target that shape — not human-sprint interim architectures ([`Agent_Timescale_Planning_Rule.mdc`](agent/Agent_Timescale_Planning_Rule.mdc)).
