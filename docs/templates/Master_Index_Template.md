...
**Pack version**: 2.7.10 *(from [`templates/VERSION`](templates/VERSION) — update on sync)*
...
- **Orchestrator git:** `orchestrator.git.mode` — **`branch-pr`**, **`branch-pr-squash`**, **`branch-push`**, **`local`**, **`current-push`** (never silent-default), or **`none`**. Asked if unset even under sync `auto-all`. See [orchestrator-git](templates/agent/roles/orchestrator-git.md).
