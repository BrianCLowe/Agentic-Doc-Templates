...
**Pack version**: 2.7.9 *(from [`templates/VERSION`](templates/VERSION) — update on sync)*
...
- **Orchestrator git:** `orchestrator.git.mode` — **`branch-pr`** (draft mid-run → build-verify → mark ready), **`branch-pr-squash`** (same + squash before ready for tip-only bots), **`branch-push`**, **`local`**, **`current-push`** (push the branch you’re on; never silent-default), or **`none`**. Asked if unset even under sync `auto-all`. See [orchestrator](templates/agent/roles/orchestrator.md).
