...
- **Summary:** Sticky outcomes. A drained child list is not capability-done. `## Outcomes` stays open until a passing exercise note. A break note does not add a second Exercise. Slices do not check operable Acceptance. Todo warden is the only role that creates a human-verify playtest, and only after that note. Sync pass `optional-todo-outcomes` retrofits existing stems and withdraws premature look-rows.
- **Changes:**
  - `VERSION` — 2.9.8 → 2.9.9
