...
- **Files:**
  ...
  - `agent/tools/` — **(new)** per-tool install/sync playbooks (`cursor`, `grok-build`, `claude-code`, `github-copilot`, `agents-md`, `openclaw`, thin `continue`/`cline`) + `README.md`; paths aligned with vendor docs (Claude `AGENTS.md` caveat; Grok `.grok/agents/` confirmed via Grok Build local docs)
