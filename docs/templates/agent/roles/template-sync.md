...
6. Summarize what changed (include reshape offered + accepted/declined + whether a separate commit was suggested).
7. Run B’s **Present unset options** — explain + ask once for any missing `optional_rules.*`. Do **not** stay silent because status is unset or because an installed tool has no adapter install.
8. **Stop.**
...
- You have not scanned live `features/` / `_shared/` unless `content-templates` or (`optional-live-reshape` and user said yes)
