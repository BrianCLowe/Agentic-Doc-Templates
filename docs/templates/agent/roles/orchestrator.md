...
6. **Bookkeep** — `[x]` + date, Current focus; dual-write `procure` / `decide` / `waiting` only. Do **not** add a human-verify playtest in this step. After verifier **pass**, if a passing exercise note exists and that outcome is still `[ ]`, run the outcome-audit check in this session. An outcome already `[x]` does not get another playtest.  
