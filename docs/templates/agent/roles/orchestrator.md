...
6. **Bookkeep** — `[x]` + date, Current focus; dual-write `procure` / `decide` / `waiting` only. Do **not** add a human-verify playtest in this step. After verifier **pass**, if a current passing note exists and that outcome is still `[ ]`, run the outcome-audit check in this session. An outcome already `[x]` does not get another playtest. If this turn’s code changed an observable that checked outcome names, the audit reopens it (uncheck, one sentence, one Exercise). A copy or typo fix does not.  
