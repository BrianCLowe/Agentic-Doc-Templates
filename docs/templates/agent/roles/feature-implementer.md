...
5. Update Understanding / spec **only if this session** changed **shape** or contract — especially **What this is NOT**. Keep Understanding shape-only (no How it should work / UI/UX / Done when). If you update Understanding, re-check this TODO vs code/spec and **uncheck** mismatches (Workflow §4); relocate any trim overflow into the spec. Otherwise leave Understanding alone.
