...
6. **Outcomes (Workflow §5.5):** Do **not** check an operable **Acceptance** line or an **Outcomes** row. A slice may meet a Behavior bullet; the outcome stays `[ ]`. New TODO items on a stem that has outcome rows end with `` `outcome: <slug>` ``. A new operable scenario adds the Acceptance line and the unchecked Outcomes row in the same turn, both open. If this unit is the last open child of an outcome and there is no passing exercise note, add one High item titled **Exercise** plus the slug — run the scenario and record the first break (path, date, observed result), label `` `outcome: <slug>` ``, and set Current focus there. While a phased stem’s domain children are still open, do not add that exercise item. If High Priority is empty but an operable Acceptance line has no open TODO and no passing note → add the exercise item. Do not report feature/stem complete while an Outcomes row is `[ ]`.
