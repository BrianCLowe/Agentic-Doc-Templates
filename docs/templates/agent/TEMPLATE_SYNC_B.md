...
5d. **Live TODO outcomes** *(if `optional-todo-outcomes`)* —
   ...
   - **On execute:** for each stem — open that spec (Overview, Behavior, Acceptance) + **this stem’s** `-TODO.md` only. Do **not** diff the repo into a task per architecture bullet. Do **not** check an Outcomes row. Do **not** reopen Completed items.
     ...
     5. If an outcome has no open child with its label, no open exercise task, and no passing note → add exactly one High item: **Exercise** plus the slug — run the scenario and record the first break (path, date, observed result), labeled `` `outcome: <slug>` ``. While a phased stem’s domain children are still open, do not add that exercise item.
