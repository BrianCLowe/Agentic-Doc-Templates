...
| `docs/templates/help/`, `docs/templates/agent/` | Setup and usage guides inside the template pack — refresh from upstream when those folders change |
...
8. If migrating from pre-1.8 layouts: run layout migration per [`BOOTSTRAP.md`](BOOTSTRAP.md) Step 0b — target is a single `docs/templates/` pack with `help/` and `agent/` subfolders (not at `docs/` root); refresh the whole `docs/templates/` tree from upstream.
