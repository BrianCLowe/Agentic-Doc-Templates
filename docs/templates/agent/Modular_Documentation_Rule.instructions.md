...
6. If work needs a **human** (procure, playtest/feel, decide/sign-off, or external waiting): **dual-write** in the same edit — owner `*-TODO.md` item **and** an Open row on **`docs/Human-TODO.md`** (Workflow §13). Optional team roster: stamp only Active `role_id`s on `docs/Team-Roster.md`; handoff agents do **not** invent bot rows (bots self-ID). Never store secrets in docs. Do not bury human asks only in feature TODOs.
