...
- **`prevent`:** **Draft `-Understanding.md` first** when scoping — agent writes; user confirms **shape** only (Workflow §4). `draft` blocks **coding**, not creating the file. Shape-only sections — not a second spec. Capture **product-defining surface/architecture identity** in is / is not when stated (module/API detail → spec). Lock obvious defaults; **Assumptions = real forks only** (empty is fine). Examples are not identity. Tell the user confirmation is **is / is not + remaining real-fork Assumptions**, not a full spec review.
