...
- Update **Current focus** + `-TODO.md` (`[x]` + date; **move** a finished plotted item into **Completed**). Do not add a Completed row for an incidental fix — git already has it (Workflow §5). Human-TODO feedback → sync owner + Human-TODO Done (§13); never mark human rows from assumptions.
- **Outcome audit (§5.5)** for stems this turn touched, when this turn finished the last open non-exercise child or an exercise item, or would claim the feature, stem, or outcome done. Follow the Outcome audit section of `docs/templates/agent/roles/todo-warden.md` in this session. Spawn `todo-warden` only when that adapter exists **and** the user ask is the warden row. Declining doc-roles does not skip the audit. An implementer unit does **not** check the Outcomes row or Acceptance — work-verifier fails a unit that checks them. Run that check after verifier **pass**, or after the unit edit when this session will not run work-verifier. An outcome already `[x]` stays `[x]`. Do not add another human-verify playtest.
