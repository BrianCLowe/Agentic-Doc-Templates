...
6. If work needs a **human** (procure, playtest/feel, decide/sign-off, or external waiting): **dual-write** — owner `*-TODO.md` **and** an Open row on **`docs/Human-TODO.md`** (Workflow §13). Stamp only Active `role_id`s; do not invent roster rows. Never store secrets in docs. A `procure` for an API the running app will call → also a **Services this app consumes** row on `docs/Tooling.md` (name, why, credential name; Workflow §11). Not a Required/Optional install row.
