...
- **Outcome audit (§5.5)** for stems this turn touched, before claiming the feature, stem, or outcome done, and when this turn finished the last open non-exercise child or an exercise item. Follow the Outcome audit section of `docs/templates/agent/roles/todo-warden.md` in this session. Spawn `todo-warden` only when that adapter exists **and** the user ask is the warden row. Declining doc-roles does not skip the audit. A slice that only looks close does not check the outcome.
