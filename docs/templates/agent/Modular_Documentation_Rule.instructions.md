...
- **`prevent`:** draft Understanding first; `draft` blocks coding. Lock obvious defaults; **Assumptions = real forks only**. Do not treat reference-doc examples as the target unless clearly set as the target. **`balanced`:** Understanding when identity is fuzzy. **`build-first`:** no Understanding required.
