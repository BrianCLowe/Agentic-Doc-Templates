...
- **Operable done (Workflow §5.3):** User/operator-facing stems need domain **and** an exercise path (UI / CLI / product API / documented smoke) unless **library-only** or a loud **phased bridge**. Product Overview/Acceptance + domain-only TODOs = under-authored. Open operable Acceptance with no TODO that addresses them = incomplete; update Acceptance when a unit meets a line. Do not treat domain/tests-only clearance as product/Layer done. **No UI specs ≠ defer UI** — scaffold + wire a minimal boring surface (or CLI/smoke) at agent speed; only defer when the user explicitly gates design-first / no UI / library-only.

**After changes (mandatory):**
- **Build & verify** when code changed: run the project’s handoff verify (`docs/Tooling.md` **Project verify** if present, else stack defaults) — fix failures before “you can test.” See installed `agent-build-verify` rule / `docs/templates/agent/Agent_Build_Verify_Rule.mdc`.
- Update **Current focus** and the relevant `-TODO.md` — `[x]` + date on completed items.
- Human-TODO feedback → sync owner TODO + `docs/Human-TODO.md` Done (Workflow §13); never mark human rows done from assumptions.
- Update Understanding / spec **only if this session** changed shape, contract, or assumptions. Understanding stays shape-only; contract → spec.
- **Preference corrections (same turn):** If the user corrected a lasting UI/interaction preference on a confirmed stem that could be “improved away,” append 1-line **Decisions** row(s) and fix contradicting Behavior / Acceptance / Visual refs (Workflow §10). Skip pure spacing unless they say “remember this.” Do **not** put these in Current focus. Do **not** wait for a session-wrap ask.
- **When updating Understanding:** relocate + TODO uncheck for that stem (Workflow §4). Reconcile vs code only on mismatch / shape change / Understanding update — not session-start audit (Workflow §4).
- Move finished items to Completed when long. Read other files only when linked from Master_Index or the current TODO/spec.
