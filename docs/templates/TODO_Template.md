...
- **Finished kit (Workflow §5.4):** If the spec names an in-scope surface, this file (or the owning stem’s TODO) must have an implementable item for it. Filling those rows is **not** inventing work. “Picked up” = Current focus / orchestrate starts the unit — not when the row is first written. Do not wait for a human to choose leftovers. New map rows only when that slice is next (Workflow §0 inventory).
